...
class fermi_dirac(distribution):
    ...
    def __init__(self, temperature, mu, background=0, integrated_weight=1, name="fermi_dirac"):
        super().__init__(name)
    ...
    def __call__(self, energy_range, mu, background, integrated_weight, energy_resolution):
        """
        TBD
        """
        ...
        return gaussian_filter(result, sigma=energy_resolution/(fwhm_to_std*step_size))
    ...
    def convolve(self, energy_range, energy_resolution):
        ...
        step_size=np.abs(energy_range[1]-energy_range[0])
        fwhm_to_std = np.sqrt(8*np.log(2)) # Conversion from FWHM to standard deviation [-]
        return gaussian_filter(self.calculate(energy_range), sigma=energy_resolution/(fwhm_to_std*step_size))
