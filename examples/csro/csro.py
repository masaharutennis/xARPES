...
bmap.correct_fermi_edge(
      hnuminPhi_guess=52.25, background_guess=1e2,
      integrated_weight_guess=1e3, angle_min=-20, angle_max=20,
...
fig = bmap.fit_fermi_edge(hnuminPhi_guess=52.25, background_guess=1e5,
                          integrated_weight_guess=1.5e5, angle_min=-20,
                          angle_max=20, ekin_min=52.20, ekin_max=52.3,
...
xarpes.Linear(offset=8.5e2, slope=9),
xarpes.SpectralLinear(amplitude=60, peak=10, broadening=0.03,
                      name='Linear_right', index='1')
])
...
self_energy = xarpes.SelfEnergy(*mdcs.expose_parameters(select_label='Linear_right_1',
                                fermi_velocity=-1.8, fermi_wavevector=0.52))
...
xarpes.Linear(offset=7e2, slope=-6),
xarpes.SpectralLinear(amplitude=55, peak=-2.5, broadening=0.04,
                      name='Linear_left', index='1'),
])

fig = plt.figure(figsize=(8, 6)); ax = fig.gca()

fig = mdc2.visualize_guess(distributions=guess_dists2, energy_value=enegy_value, ax=ax)
...
fermi_velocity=2.6, fermi_wavevector=-0.1))
